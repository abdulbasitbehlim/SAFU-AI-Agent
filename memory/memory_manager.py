...
def format_memory_for_prompt(memory: dict | None) -> str:
    ...
    for field in _IDENTITY_FIELDS:
        ...
        if not val:
            continue
        if field == "language":
            # Historical observation only. SAFU's output language is locked to English.
            core_lines.append(
                f"Previously detected input language: {val} (history only; always respond in English)")
        else:
...
def all_entries_for_ui() -> list[dict]:
    """Flat list for the memory panel: what SAFU knows, and when it learned it.
