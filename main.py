...
# (screen_process, close_camera, save_memory, manage_monitor, shutdown_safu,
# system_status).
...
from memory.config_manager     import (
    get_brief_enabled, get_voice, get_wake_word_enabled, save_wake_word_enabled, get_dashboard_enabled,    get_input_device, get_output_device,
)
...
# Voice-priority default. Gemini 2.5 Flash Live is intentionally used because
# this build relies on incremental send_client_content plus proactive audio,
# both of which are supported together on the 2.5 Live model. The model can be
# overridden with SAFU_LIVE_MODEL for development/testing, but other Live model
# families may require API-behaviour changes in this client.
...
CHUNK_SIZE          = 640   # 40 ms mic chunks: lower turn latency with modest CPU cost
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are SAFU, Basit's personal AI assistant. "
            "Understand the user in any language, but speak and write every reply in English only. "
            "Be concise, direct, and always use the provided tools to complete tasks. "
            "Never simulate or guess results — always call the appropriate tool."
        )

def _low_latency_realtime_config(silence_ms: int = 180):
    """Build a Gemini Live VAD config that ends a user's turn quickly.

    Google Live defaults are intentionally conservative.  A shorter silence
    window removes the noticeable pause after the user stops speaking.  The
    helper is capability-checked so older google-genai builds still start.
    """
    try:
        silence_ms = max(100, min(900, int(silence_ms)))
        aad_kwargs = dict(
            disabled=False,
            prefix_padding_ms=30,
            silence_duration_ms=silence_ms,
        )
        _start = getattr(types, "StartSensitivity", None)
        _end = getattr(types, "EndSensitivity", None)
        if _start is not None and hasattr(_start, "START_SENSITIVITY_HIGH"):
            aad_kwargs["start_of_speech_sensitivity"] = _start.START_SENSITIVITY_HIGH
        if _end is not None and hasattr(_end, "END_SENSITIVITY_HIGH"):
            aad_kwargs["end_of_speech_sensitivity"] = _end.END_SENSITIVITY_HIGH

        aad_cls = getattr(types, "AutomaticActivityDetection", None)
        rt_cls = getattr(types, "RealtimeInputConfig", None)
        if aad_cls is None or rt_cls is None:
            return None
        rt_kwargs = {"automatic_activity_detection": aad_cls(**aad_kwargs)}
        _handling = getattr(types, "ActivityHandling", None)
        if _handling is not None and hasattr(_handling, "START_OF_ACTIVITY_INTERRUPTS"):
            rt_kwargs["activity_handling"] = _handling.START_OF_ACTIVITY_INTERRUPTS
        return rt_cls(**rt_kwargs)
    except Exception as e:
        print(f"[SAFU] Low-latency VAD config unavailable: {e}")
        return None
...
TOOL_DECLARATIONS = [
    ...
    # core.action_loader (see SafuLive.__init__).
    {
        "name": "system_status",
        "description": (
    ...
    },
    {
        ...
        "description": (
            ...
            "SAFU checks these topics once a day and alerts the user when there is a new development. "
    ...
    },
    {
        ...
        "description": (
            ...
            "close the assistant, say goodbye, or stop Safu. "
    ...
    },
    {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "value": {"type": "STRING", "description": "Concise value in English (e.g. Basit, pizza, older sister)"},
            },
...
class SafuLive:
    def __init__(self, ui: SafuUI):
        self.ui             = ui
        self._asst_name     = "SAFU"   # updated each session from config
        self.session              = None
        ...
        # Proactive-audio gating is useful in open-room demos, but it adds another
        # server-side decision before ordinary replies. Wake-word gating already protects
        # SAFU from background speech, so Instant Reply mode keeps it OFF by default.
        try:
            _perf_cfg = json.loads(API_CONFIG_PATH.read_text(encoding="utf-8"))
            self._enhanced_live = bool(_perf_cfg.get("proactive_audio", False))
        except Exception:
    ...
    def _wake_state(self) -> dict:
        # Wake is always available while a Gemini session is connected.  If the
        # optional PocketSphinx engine exists it stays 100% local; otherwise the
        # app uses Gemini input transcription only to spot the phrase and blocks
        # all audio/tool output until the phrase is heard.
        local = bool(self._wake_detector and self._wake_detector.ready) or wake_is_ready()
        return {"enabled": self._wake_enabled, "awake": self._awake,
                "ready": True, "mode": "local" if local else "cloud"}

    def _ensure_wake_detector(self) -> bool:
        """Start the optional local detector; return False to use cloud fallback."""
    ...
    def _on_wake_detected(self) -> None:
        """Called from the detector thread when 'Hey Safu' is heard."""
    ...
    def wake(self, reason: str = "wake word") -> None:
        ...
        # If Safu started asleep in wake-word mode, deliver the once-per-launch
        # morning brief after the first real wake instead of silently skipping it.
        if (not self._briefing_sent and get_brief_enabled()
                and 5 <= datetime.now().hour < 12 and self.session):
            loop = getattr(self, "_loop", None)
            if loop:
                self._briefing_sent = True
                try:
                    asyncio.run_coroutine_threadsafe(self._send_startup_briefing(), loop)
                except Exception as e:
                    self._briefing_sent = False
                    print(f"[Briefing] Could not schedule after wake: {e}")
    ...
    def sleep(self, reason: str = "timeout") -> None:
        ...
        self.ui.write_log(f"SYS: Sleeping — {reason}. Say 'Hey Safu' to wake me.")
    ...
    def _ui_wake_toggle(self, enable: bool) -> str:
        """Enable/disable Hey Safu. No separate wake-model download is required."""
        if enable:
            self._wake_enabled = True
            save_wake_word_enabled(True)
            local = self._ensure_wake_detector()
            self.sleep(reason="wake word enabled")
            if not local:
                self.ui.write_log(
                    "SYS: Hey Safu enabled in download-free cloud fallback mode. "
                    "Run full setup later if you want wake detection to stay fully local."
                )
            return "enabled"
    ...
    def _ui_wake_install(self) -> tuple[bool, str]:
        """Install/repair the local Hey Safu keyword engine (UI worker thread)."""
        return wake_install(logger=lambda m: self.ui.write_log(f"SYS: {m}"))

    def plugin_say(self, instruction: str) -> None:
        """
        Thread-safe speech channel for plugins: lets a plugin ask SAFU to
        ...
        English only. Silently a no-op when no session is connected.
        """
        loop = getattr(self, "_loop", None)
        ...
        async def _say():
            try:
                await self.session.send_client_content(
                    turns={"role": "user", "parts": [{"text": "[ENGLISH ONLY] " + instruction}]},
                    turn_complete=True,
                )
            except Exception as e:
                print(f"[PluginSay] {e}")
    ...
    def _on_text_command(self, text: str):
        ...
        # "Hey Safu" or the WAKE NOW button.
        if self._wake_enabled and not self._awake:
            self.ui.write_log("SYS: I'm asleep — say 'Hey Safu' or tap WAKE NOW first.")
            return
    ...
    def interrupt(self) -> None:
        """Stop SAFU mid-speech: drain queued audio and open mic immediately."""
        ...
        if q:
            ...
            while True:
                try:
                    ...
                    break
            if drained:
                print(f"[SAFU] ✋ Interrupted — {drained} audio chunks discarded")
        self.set_speaking(False)
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "SAFU").strip()
            _user_name = (_cfg.get("user_name") or "").strip()
        except Exception:
            self._asst_name = "SAFU"
            _user_name = ""
            _cfg = {}

        instant_mode = bool(_cfg.get("instant_reply_mode", True))
        try:
            vad_silence_ms = int(_cfg.get("vad_silence_ms", 180))
        except Exception:
            vad_silence_ms = 180
        try:
            reply_token_cap = int(_cfg.get("reply_token_cap", 512))
        except Exception:
            reply_token_cap = 512
        reply_token_cap = max(128, min(2048, reply_token_cap))
        ...
        _addr = (f"ADDRESS: Always call the user '{_user_name}'."
                 if _user_name
                 else "ADDRESS: Speak English only; address the user as 'sir' naturally when appropriate.")
        language_ctx = (
            "[LANGUAGE POLICY]\n"
            "INPUT IS MULTILINGUAL: understand the user's spoken or typed input in whatever language "
            "it arrives. Detect and interpret the input language automatically; do not assume the user "
            "spoke English and do not reject Hindi, Hinglish, Urdu, Korean, Arabic, Spanish, or any other "
            "supported language.\n"
            "OUTPUT IS ENGLISH ONLY: every spoken and written SAFU response must be natural English. "
            "Never switch the reply voice or normal chat output to Korean or another language because of "
            "automatic language detection, memory, tool output, or the user's input language. You may quote "
            "foreign-language text only when it is necessary for translation or analysis, but explanations "
            "and conversation must remain in English.\n\n"
        )
        ...
        identity_ctx = (
            ...
            f"{_addr}\n"
            "VOICE PERSONA: sound warm, youthful, lively, confident and expressive, "
            "with a friendly anime-assistant energy. This is an original persona: never "
            "claim to be, or imitate, Sakura Haruno or any real voice actor.\n\n"
        )

        parts = [language_ctx, time_ctx, identity_ctx]
        if mem_str:
        ...
        cfg = dict(
            ...
            # Keep answers compact by default so speech begins and finishes quickly.
            # Complex/heavy tasks still use tools and can provide detailed results in the UI.
            max_output_tokens=reply_token_cap if instant_mode else 1024,
            temperature=0.35 if instant_mode else 0.55,
            output_audio_transcription={},
            # No language_codes on purpose: user speech stays multilingual/auto-detected.
            ...
            # window — SAFU can stay in one conversation for hours
        ...
        if instant_mode:
            _rt_cfg = _low_latency_realtime_config(vad_silence_ms)
            if _rt_cfg is not None:
                cfg["realtime_input_config"] = _rt_cfg
        if self._enhanced_live:
            # Proactive audio: SAFU can decide not to answer irrelevant room
            # speech. This is supported by the default Gemini 2.5 Flash Live
            # model on the v1beta endpoint. If an overridden model rejects this
            # field, the reconnect logic below disables it automatically.
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[SAFU] 📤 {name} → {str(result)[:80]}")
        return types.FunctionResponse(
            id=fc.id, name=name,
    ...
    async def _listen_audio(self):
        print("[SAFU] 🎤 Mic started")
        loop = asyncio.get_event_loop()

        def callback(indata, frames, time_info, status):
            # ── Wake-word gate ───────────────────────────────────────────────
            # With the local PocketSphinx detector ready, sleeping audio stays on
            # device and is fed only to that detector thread. If the optional local
            # engine is unavailable, the clearly-labelled cloud fallback streams
            # audio only for transcription; assistant audio and tool calls remain
            # blocked until "Hey Safu" is detected.
            if self._wake_enabled and not self._awake:
                det = self._wake_detector
                if det is not None and det.ready:
                    det.feed(indata)
                    return
                # Download-free fallback: stream only for input transcription.
                # _receive_audio blocks all assistant audio and tool execution
                # while asleep, so background speech cannot trigger actions.
                data = indata.tobytes()
                try:
                ...
                return
            with self._speaking_lock:
                safu_speaking = self._is_speaking
            if not safu_speaking and not self.ui.muted and not self._phone_active:
                data = indata.tobytes()
                loop.call_soon_threadsafe(
                    self.out_queue.put_nowait,
                    {"data": data, "mime_type": "audio/pcm;rate=16000"}
        ...
        try:
            ...
            if _mic_dev is not None:
                print(f"[SAFU] 🎤 Input device: {_mic_name}")
            try:
                ...
                if _mic_dev is None:
                    raise
                print(f"[SAFU] ⚠️  Mic '{_mic_name}' failed: {_e} — using default")
                self.ui.write_log(
                ...
                async for response in self.session.receive():
                    ...
                    if _sru is not None:
                        if getattr(_sru, "resumable", False) and getattr(_sru, "new_handle", None):
                            if self._resume_handle is None:
                                print("[SAFU] 🔗 Session resumption armed")
                            self._resume_handle = _sru.new_handle

                    if response.data:
                        if self._wake_enabled and not self._awake:
                            # Cloud-wake fallback may receive model audio while it
                            # is transcribing room speech. Never play any of it
                            # until the wake phrase has actually been detected.
                            pass
                    ...
                    if response.server_content:
                        ...
                        if sc.output_transcription and sc.output_transcription.text:
                            ...
                            if (not self._wake_enabled or self._awake) and txt and txt != (out_buf[-1] if out_buf else ""):
                                out_buf.append(txt)

                        if sc.input_transcription and sc.input_transcription.text:
                            txt = _clean_transcript(sc.input_transcription.text)
                            if txt:
                                # No-download wake fallback. Punctuation is stripped
                                # so variants like "Hey, Safu" still match.
                                if self._wake_enabled and not self._awake:
                                    _wake_txt = re.sub(r"[^a-z0-9]+", " ", txt.lower()).strip()
                                    if "hey safu" in _wake_txt:
                                        self.wake(reason="Hey Safu (cloud wake)")
                                        in_buf = []
                                if not self._wake_enabled or self._awake:
                                    in_buf.append(txt)
                        ...
                        if sc.turn_complete:
                            ...
                            if self._pending_vision and self.session:
                                ...
                                if self._vision_cam_active:
                                    # Camera: keep busy until SAFU finishes speaking the answer
                    ...
                    if response.tool_call:
                        ...
                        for fc in response.tool_call.function_calls:
                            if self._wake_enabled and not self._awake:
                                # Security gate for cloud-wake fallback: background
                                # conversations can be transcribed, but can never
                                # execute a tool while the assistant is asleep.
                                fr = types.FunctionResponse(
                                    id=fc.id, name=fc.name,
                                    response={"result": "Blocked: assistant is asleep. Say Hey Safu first."},
                                )
                            else:
                                print(f"[SAFU] 📞 {fc.name}")
    ...
    async def _play_audio(self):
        ...
        try:
            ...
            if _spk_dev is None:
                raise
            print(f"[SAFU] ⚠️  Output device '{_spk_name}' failed: {_e} — using default")
        ...
        try:
            while True:
                ...
                # Drive the HUD waveform from SAFU's own voice while speaking.
                try:
    ...
    async def _send_startup_briefing(self) -> None:
        ...
        lang = "English"  # SAFU is intentionally English-only
        name = _val("name")
        time_str = datetime.now().strftime("%H:%M")

        # Start fetching news immediately — runs in parallel while phase 1 plays
        loop = asyncio.get_event_loop()
        news_future = loop.run_in_executor(None, _fetch_news_sync, "top world, India, science, technology and business news today")
        ...
        # SAFU is English-only by user preference; startup never follows detected language.
        lang_clause = " Speak this greeting in English only."
        ...
        async def _deliver_news():
            try:
                lang_str = " Speak in English only, regardless of detected input language."
    ...
    async def _run_system_monitor(self) -> None:
        ...
        while True:
            ...
            try:
                await self.session.send_client_content(
                    turns={"role": "user", "parts": [{"text": "[ENGLISH ONLY] " + alert}]},
                    turn_complete=True,
                )
            except Exception as e:
    ...
    async def _run_background_monitor(self) -> None:
        ...
        while True:
            if self.session and self._awake:
                # Don't interrupt if user spoke recently or SAFU is mid-sentence
                with self._speaking_lock:
                    speaking = self._is_speaking
                ...
                if not speaking and not recent_speech:
                    try:
                        ...
                        for alert in alerts:
                            msg = (
                                f"{alert}\n\n"
                                "Inform the user about this development naturally in English only. "
    ...
    async def _run_proactive_mode(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                prompt = "[ENGLISH ONLY] " + self._proactive.build_prompt(
                    memory       = memory,
        ...
        while True:
            try:
                ...
                if self.session:
                    ...
                    # has no desktop WAKE button — so it wakes SAFU if asleep.
                    if self._wake_enabled and not self._awake:
    ...
    async def run(self):
        ...
        # Remote phone dashboard is opt-in. Keeping it off saves background RAM/CPU
        # and avoids opening a LAN service/firewall rule unless explicitly requested.
        if get_dashboard_enabled():
            try:
            ...
            self._dashboard = None
            print("[Dashboard] Off (low-resource default). Enable remote_dashboard_enabled when needed.")

        while True:
            try:
                print("[SAFU] Connecting...")
                self.ui.set_state("THINKING")
                _resumed_with = self._resume_handle is not None
                config = self._build_config()

                # Fresh client on every reconnect avoids stale HTTP session state.
                # Proactive audio for Gemini 2.5 Flash Live is documented on v1beta.
                # Keep the API version stable; if the selected model rejects the
                # proactivity field we reconnect once with that field disabled.
                client = genai.Client(
                ...
                async with (
                    ...
                    # until the user says "Hey Safu" or taps wake in the UI.
                    if self._wake_enabled:
                        _local_wake = self._ensure_wake_detector()
                        self._awake = False
                        self.ui.set_state("SLEEPING")
                        _mode = "local" if _local_wake else "download-free cloud fallback"
                        self.ui.write_log(f"SYS: SAFU online — sleeping ({_mode}). Say 'Hey Safu' to wake me.")
                    else:
                        self._awake = True
                        self.ui.set_state("LISTENING")
                        self.ui.write_log("SYS: SAFU online.")
                ...
                if _is_reconnect_signal(e):
                    print("[SAFU] Voluntary reconnect requested.")
                ...
                if _resumed_with and (
                    ...
                    print("[SAFU] 🔗 Resumption handle rejected — starting a fresh session")
                ...
                err_str = str(e)
                print(f"[SAFU] Error ({type(e).__name__}): {e}")
                traceback.print_exc()
                ...
                if any(k in err_str.lower() for k in (
                    ...
                    print("[SAFU] New API key saved — reconnecting...")
                    _conn_backoff = 3
                    continue
            ...
            print(f"[SAFU] Reconnecting in {delay}s...")
            await asyncio.sleep(delay)

def main():
    ui = SafuUI(str(Path(__file__).resolve().parent / "config" / "safu_avatar.png"))

    def runner():
        ui.wait_for_api_key()
        safu = SafuLive(ui)
        try:
            asyncio.run(safu.run())
        except KeyboardInterrupt:
            print("\n🔴 Shutting down...")
