"""
dashboard/server.py — SAFU Local HTTP Dashboard

Plain HTTP on port 8000 (no SSL warnings, firewall permissions remain user-controlled).
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "SAFU Uploads",
        Path.home() / "Documents" / "SAFU Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# Firewall/network-profile changes are intentionally not automated.
# The remote dashboard is opt-in and any OS permission remains user-controlled.
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "safu.key").exists() and (certs / "safu.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in SAFU to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('safu_token','{tok}');
  sessionStorage.setItem('safu_key','{key}');
  localStorage.setItem('safu_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to SAFU…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "safu.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "safu.crt"
        # Do not modify firewall/network-profile settings automatically.
        # If Windows asks whether Python may accept private-network connections,
        # the user can approve that normal OS prompt explicitly.
        cfg = uvicorn.Config(
    ...
    async def serve(self) -> None:
        ...
        # Security: Safu never silently opens firewall ports or changes a Public
        # Windows network to Private. The remote dashboard is already opt-in;
        # any OS firewall permission is left to the user's explicit approval.
        ...
        print("[Dashboard] Press 'Remote Control' in SAFU UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
