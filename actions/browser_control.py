...
try:
    ...
    # Lightweight installs can still open/search in the user's normal browser.
    # Only interactive DOM automation requires the optional full install.
...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".safu_profiles" / browser
...
class _BrowserSession:
    ...
    def start(self):
        if not _PLAYWRIGHT:
            raise RuntimeError(
                "Interactive browser automation is not installed. "
                "Run: python setup.py --full"
            )
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".safu_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using SAFU profile")
                safu = str(Path.home() / ".safu_profiles" / "firefox_safu")
                Path(safu).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(safu, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".safu_profiles" / "safari")
        ...
        # automation). Fall back to a persistent SAFU automation profile —
        # accounts logged in here once stay logged in on later sessions too.
        safu_profile = str(Path.home() / ".safu_profiles" / self.browser_name)
        Path(safu_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with SAFU profile: {safu_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(safu_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with SAFU profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(user_folder("desktop") / "safu_screenshot.png")
