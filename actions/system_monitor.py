...
def _get_gpu_usage() -> float:
    # Cache pynvml module/device handle instead of reinitialising NVML every poll.
    global _pynvml_mod, _pynvml_handle, _pynvml_ok
    if _pynvml_ok is not False:
        try:
            if _pynvml_handle is None:
                ...
                _pynvml_ok = True
            return float(_pynvml_mod.nvmlDeviceGetUtilizationRates(_pynvml_handle).gpu)
        except Exception:
            _pynvml_ok = False
    return _nvml_gpu()
    ...
    def check(self) -> str | None:
        ...
        if cpu >= self.thresholds["cpu"]:
            ...
            if self._cpu_streak >= _CPU_STREAK and self._can_alert("cpu"):
                alerts.append(
                    ...
                    "for several seconds. Warn the user in English and suggest "
        ...
        if ram >= self.thresholds["ram"] and self._can_alert("ram"):
            alerts.append(
                ...
                "Warn the user in English and suggest freeing memory."
            )
            self._record("ram")
        ...
        if temp > 0 and temp >= self.thresholds["temp"] and self._can_alert("temp"):
            alerts.append(
                ...
                "Warn the user in English and advise reducing system load "
                "or checking cooling."
            )
            self._record("temp")
        ...
        if gpu >= 0 and gpu >= self.thresholds["gpu"] and self._can_alert("gpu"):
            alerts.append(
                ...
                "Briefly inform the user in English."
            )
            self._record("gpu")
