...
def _news(query: str) -> str:
    """Current-news fetch with dated query, article-first results and Gemini fallback.

    DDG News is preferred because it returns discrete recent articles/sources. Gemini
    grounded search is a fallback, not a race where a lower-quality response can win.
    """
    from datetime import datetime
    today = datetime.now().strftime("%B %d, %Y")
    topic = query.strip() if query else "top world news"
    ddg_query = f"{topic} {today}"
    try:
        results = _ddg_news(ddg_query, max_results=8)
        if results:
            return _format_news(f"{topic} — {today}", results)
    except Exception as e:
        print(f"[WebSearch] DDG news failed ({e})")

    try:
        return _gemini_search(
            f"Today is {today}. Give the latest verified {topic}. "
            "Use current Google Search grounding, provide 5 concise headlines with source names and dates. "
            "Do not include stale stories unless they materially developed today."
        )
    except Exception as e:
        print(f"[WebSearch] Gemini news failed ({e})")
        return f"News could not be fetched right now for: {topic}"
