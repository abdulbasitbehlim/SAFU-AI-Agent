...
def _launch_windows(app_name: str) -> bool:
    # Never interpolate model/user text into cmd.exe.  Resolve real executables
    # and use shell=False; registered URI schemes go through os.startfile.
    candidate = shutil.which(app_name) or shutil.which(app_name.split(".")[0])
    if candidate:
        try:
            subprocess.Popen(
                [candidate],
    ...
    if ":" in app_name:
        import re
        # Permit normal URI schemes, but reject shell metacharacters entirely.
        if re.match(r"^[A-Za-z][A-Za-z0-9+.-]*:[^&|<>\"']+$", app_name):
            try:
