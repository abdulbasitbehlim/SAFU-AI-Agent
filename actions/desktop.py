...
def _validate_generated_code(code: str) -> tuple[bool, str]:
    """Reject imports, dunder access, dynamic execution and filesystem deletion before exec."""
    import ast
    try:
        tree = ast.parse(code)
    except SyntaxError as e:
        return False, f"invalid generated code: {e}"
    ...
    forbidden_attrs = {
        ...
        "symlink_to", "hardlink_to", "open", "truncate",
    }
    lowered = code.lower()
    risky_tokens = (
        "powershell", "cmd.exe", "regedit", "mshta", "rundll32", "certutil",
        "shutdown ", "format ", "diskpart", "\\.\\physicaldrive",
    )
    if any(tok in lowered for tok in risky_tokens):
        return False, "blocked command-launch or destructive token"

    for node in ast.walk(tree):
        if isinstance(node, forbidden_nodes):
            return False, f"blocked syntax: {type(node).__name__}"
        if isinstance(node, ast.Name) and node.id.startswith("__"):
            return False, "dunder names are blocked"
...
def _execute_generated_code(code: str, player=None) -> str:
    ...
    ok, why = _validate_generated_code(code)
    if not ok:
        return f"This generated desktop action was blocked for safety: {why}."
    ...
    try:
        exec(compile(code, "<safu_desktop>", "exec"), sandbox)
