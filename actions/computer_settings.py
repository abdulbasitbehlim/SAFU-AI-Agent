...
def brightness_up():
    if _OS == "Darwin":
        ...
        if subprocess.run(["which", "brightnessctl"],
            ...
            try:
                out = subprocess.check_output(["xrandr", "--current"], text=True, timeout=4)
                display = next(line.split()[0] for line in out.splitlines() if " connected" in line)
                verbose = subprocess.check_output(["xrandr", "--verbose"], text=True, timeout=4)
                cur = float(verbose.split("Brightness:", 1)[1].split()[0])
                subprocess.run(["xrandr", "--output", display, "--brightness", str(min(1.0, cur + 0.1))],
    ...
    if _OS == "Darwin":
        ...
        if subprocess.run(["which", "brightnessctl"],
            ...
            try:
                ...
                verbose = subprocess.check_output(["xrandr", "--verbose"], text=True, timeout=4)
                cur = float(verbose.split("Brightness:", 1)[1].split()[0])
                subprocess.run(["xrandr", "--output", display, "--brightness", str(max(0.1, cur - 0.1))],
                               capture_output=True, timeout=4)
            except Exception:
                pass
    else:
...
_IRREVERSIBLE = {
    ...
    "toggle_wifi": ("Switch WiFi off or on",
                    "If this switches WiFi off, SAFU loses its connection and "
