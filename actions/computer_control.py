...
def _safe_screenshot_path(requested: str | None) -> Path:
    fallback = user_folder("desktop") / "safu_screenshot.png"
    if not requested:
        return fallback
    try:
...
def _focus_window(title: str) -> str:
    ...
    if os_name == "windows":
        # Do not interpolate model/user text into PowerShell. Enumerate windows
        # with pywin32 and focus a matching visible title directly.
        try:
            import win32con
            import win32gui
            needle = (title or "").strip().casefold()
            if not needle:
                return "focus_window: title is empty"
            matches = []
            def _enum(hwnd, _):
                try:
                    if win32gui.IsWindowVisible(hwnd):
                        caption = win32gui.GetWindowText(hwnd) or ""
                        if needle in caption.casefold():
                            matches.append((hwnd, caption))
                except Exception:
                    pass
            win32gui.EnumWindows(_enum, None)
            if not matches:
                return f"No visible window matched: {title}"
            hwnd, caption = matches[0]
            try:
                if win32gui.IsIconic(hwnd):
                    win32gui.ShowWindow(hwnd, win32con.SW_RESTORE)
                win32gui.SetForegroundWindow(hwnd)
            except Exception:
                # A click-to-focus fallback works when Windows foreground-lock
                # policy refuses SetForegroundWindow from a background thread.
                try:
                    import pyautogui
                    left, top, right, bottom = win32gui.GetWindowRect(hwnd)
                    pyautogui.click(max(left + 20, 1), max(top + 12, 1))
                except Exception:
                    raise
            time.sleep(0.25)
            return f"Focused window: {caption or title}"
        except Exception as e:
            return f"focus_window (Windows) failed: {e}"

    if os_name == "mac":
        # Pass the requested title as argv instead of interpolating it into
        # AppleScript source. This avoids quote/script injection through a name.
        script = (
            'on run argv\n'
            'set wanted to item 1 of argv\n'
            'tell application "System Events"\n'
            'set frontmost of (first process whose name contains wanted) to true\n'
            'end tell\n'
            'end run'
        )
        try:
