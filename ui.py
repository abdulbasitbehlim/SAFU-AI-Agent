...
# Application title/version is defined in one place so UI labels stay consistent.
APP_VERSION  = "SAFU 0.0.1"
...
class C:
    # Safu signature palette: matte black + champagne gold + sakura pink.
    BG        = "#09080b"
    PANEL     = "#111015"
    PANEL2    = "#19161b"
    BORDER    = "#332b28"
...
class _SysMetrics:
    ...
    def _update(self):
        ...
        self._slow_tick = (self._slow_tick + 1) % 4
...
class HudCanvas(QWidget):
    """Animated SAFU character stage.

    The old circular reactor/orb HUD was intentionally removed.  This widget now
    presents the assistant as a lightweight animated character card: the avatar
    subtly floats and breathes, reacts to the live audio amplitude, shows a
    speech/listening waveform, and uses small sakura-petal particles.  The
    animation is deliberately low-cost (20 FPS active, ~5 FPS idle) so the UI
    does not burn CPU just to look alive.
    """

    def __init__(self, face_path: str, assistant_name: str = "SAFU", parent=None):
        ...
        self._avatar_cache: QPixmap | None = None
        self._avatar_cache_key: tuple[int, int] | None = None
        self._petals: list[list[float]] = []
        self._blink = True
        self._blink_tick = 0
        self._paint_tick = 0
        ...
        self._tmr.start(50)   # max 20 FPS; idle repaint is throttled further
    ...
    def _load_face(self, path: str):
        """Load the original transparent character art without circular masking."""
        px = QPixmap(path)
        self._avatar_px = None if px.isNull() else px
    ...
    def _step(self):
        self._tick += 1
        self._live_amp *= 0.84
        self._amp_disp += (self._live_amp - self._amp_disp) * 0.38
        amp = self._amp_disp

        # A few drifting petals give the character some life without costly
        # full-scene particle simulation.
        if random.random() < (0.045 if self.speaking else 0.015):
            self._petals.append([
                random.uniform(0.08, 0.92) * max(1, self.width()),
                -8.0,
        ...
        nxt = []
        for x, y, vx, vy, alpha, size in self._petals:
            x += vx
            y += vy
            alpha -= 0.0035
            if y < self.height() + 14 and alpha > 0.05:
                nxt.append([x, y, vx, vy, alpha, size])
        self._petals = nxt[-28:]
        ...
        # Paint every tick while active; otherwise one in four ticks (~5 FPS).
        self._paint_tick = (self._paint_tick + 1) % 4
        active = self.speaking or amp > 0.02 or self.state in ("THINKING", "PROCESSING", "LISTENING")
        if active or blinked or self._paint_tick == 0:
            self.update()

    def _draw_wave(self, p: QPainter, x: float, y: float, width: float, height: float, amp: float):
        n = 28
        gap = 3.0
        bw = max(2.0, (width - gap * (n - 1)) / n)
        mid = (n - 1) / 2.0
        for i in range(n):
            env = max(0.18, 1.0 - abs(i - mid) / max(1.0, mid))
            wave = 0.5 + 0.5 * math.sin(self._tick * 0.42 + i * 0.85)
            idle = 0.12 + 0.08 * math.sin(self._tick * 0.16 + i * 0.55)
            level = idle + amp * env * (0.45 + 0.75 * wave)
            if self.speaking:
                level += 0.16 * env * wave
            h = max(3.0, min(height, height * level))
            col = C.MUTED_C if self.muted else (C.ACC if self.speaking else C.PRI)
            p.setBrush(QBrush(qcol(col, 210 if amp > 0.03 or self.speaking else 115)))
            p.setPen(Qt.PenStyle.NoPen)
            p.drawRoundedRect(QRectF(x + i * (bw + gap), y + (height - h) / 2, bw, h), 2, 2)
    ...
    def paintEvent(self, _):
        ...
        # Soft black/sakura/gold stage instead of the old sci-fi circular HUD.
        bg = QLinearGradient(0, 0, W, H)
        bg.setColorAt(0.0, qcol("#09080b"))
        bg.setColorAt(0.55, qcol("#120e12"))
        bg.setColorAt(1.0, qcol("#17110d"))
        p.fillRect(self.rect(), QBrush(bg))

        # Gold top accent and faint inner border.
        p.fillRect(QRectF(0, 0, W, 2), qcol(C.PRI, 210))
        p.setPen(QPen(qcol(C.BORDER_B, 120), 1))
        p.setBrush(Qt.BrushStyle.NoBrush)
        p.drawRoundedRect(QRectF(8, 8, max(1, W-16), max(1, H-16)), 18, 18)

        # Ambient glows behind the character.
        glow = QRadialGradient(W * 0.64, H * 0.52, max(W, H) * 0.52)
        glow.setColorAt(0.0, qcol(C.ACC, 58 if self.speaking else 35))
        glow.setColorAt(0.55, qcol(C.PRI, 22))
        glow.setColorAt(1.0, qcol(C.BG, 0))
        p.fillRect(self.rect(), QBrush(glow))

        # Left-side conversational card.
        card_x = 26.0
        card_y = 30.0
        card_w = max(210.0, min(W * 0.42, 330.0))
        card_h = min(190.0, H * 0.42)
        p.setBrush(QBrush(qcol("#100e12", 225)))
        p.setPen(QPen(qcol(C.BORDER_B, 165), 1))
        p.drawRoundedRect(QRectF(card_x, card_y, card_w, card_h), 18, 18)

        p.setFont(QFont("Segoe UI", 10, QFont.Weight.DemiBold))
        p.setPen(QPen(qcol(C.TEXT_DIM), 1))
        p.drawText(QRectF(card_x + 18, card_y + 16, card_w - 36, 20),
                   Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter,
                   "PERSONAL AI  •  BUILT FOR BASIT")

        p.setFont(QFont("Segoe UI", 25, QFont.Weight.Bold))
        p.setPen(QPen(qcol(C.WHITE), 1))
        p.drawText(QRectF(card_x + 18, card_y + 42, card_w - 36, 42),
                   Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter,
                   self._assistant_name)

        if self.muted:
            status, scol = "MIC MUTED", C.MUTED_C
        elif self.speaking:
            status, scol = "SPEAKING WITH YOU", C.ACC
        elif self.state == "THINKING":
            status, scol = "THINKING", C.ACC2
        elif self.state == "PROCESSING":
            status, scol = "WORKING", C.ACC2
        elif self.state == "SLEEPING":
            status, scol = "SLEEPING • SAY ‘HEY SAFU’", C.TEXT_DIM
        elif self.state == "LISTENING":
            status, scol = "LISTENING", C.GREEN
        else:
            status, scol = self.state, C.PRI

        p.setBrush(QBrush(qcol(scol, 28)))
        p.setPen(QPen(qcol(scol, 170), 1))
        p.drawRoundedRect(QRectF(card_x + 18, card_y + 92, card_w - 36, 31), 10, 10)
        p.setFont(QFont("Segoe UI", 9, QFont.Weight.DemiBold))
        p.setPen(QPen(qcol(scol), 1))
        p.drawText(QRectF(card_x + 31, card_y + 94, card_w - 62, 27),
                   Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter,
                   ("● " if self._blink else "○ ") + status)

        self._draw_wave(p, card_x + 18, card_y + 140, card_w - 36, 28, self._amp_disp)

        # Character art. It floats by a few pixels and subtly scales with voice
        # amplitude; no circular crop, ring or reactor is drawn around it.
        if self._avatar_px:
            max_h = int(max(160, H - 26))
            max_w = int(max(160, W * 0.62))
            scale = 1.0 + (0.018 if self.speaking else 0.006) + self._amp_disp * 0.026
            target_h = int(max_h * min(1.05, scale))
            target_w = max_w
            qh = max(16, (target_h // 8) * 8)
            qw = max(16, (target_w // 8) * 8)
            key = (qw, qh)
            if self._avatar_cache is None or self._avatar_cache_key != key:
                self._avatar_cache = self._avatar_px.scaled(
                    qw, qh,
                    Qt.AspectRatioMode.KeepAspectRatio,
                    Qt.TransformationMode.SmoothTransformation,
                )
                self._avatar_cache_key = key
            av = self._avatar_cache
            bob = math.sin(self._tick * (0.22 if self.speaking else 0.08)) * (4.5 if self.speaking else 2.0)
            ax = int(W - av.width() - 8)
            ay = int(H - av.height() + bob)
            p.drawPixmap(ax, ay, av)
        else:
            # Graceful fallback: still no orb — just a clean monogram panel.
            r = min(140, int(min(W, H) * 0.22))
            cx, cy = W * 0.68, H * 0.50
            p.setBrush(QBrush(qcol(C.PANEL2, 230)))
            p.setPen(QPen(qcol(C.PRI, 180), 2))
            p.drawRoundedRect(QRectF(cx-r, cy-r, r*2, r*2), 26, 26)
            p.setFont(QFont("Segoe UI", 32, QFont.Weight.Bold))
            p.setPen(QPen(qcol(C.PRI), 1))
            p.drawText(QRectF(cx-r, cy-24, r*2, 48), Qt.AlignmentFlag.AlignCenter, "S")

        # Petals are intentionally tiny and sparse to keep the animation cheap.
        for x, y, _vx, _vy, alpha, size in self._petals:
            p.setPen(Qt.PenStyle.NoPen)
            p.setBrush(QBrush(qcol(C.ACC, int(150 * alpha))))
            p.drawEllipse(QRectF(x, y, size * 1.4, size))

        # Small bottom hint gives the user an immediately discoverable wake path.
        hint = "Say ‘Hey Safu’  •  type a command  •  drop a file  •  Esc to interrupt"
        p.setFont(QFont("Segoe UI", 8))
        p.setPen(QPen(qcol(C.TEXT_DIM, 180), 1))
        p.drawText(QRectF(22, H - 29, W - 44, 18),
...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        """)
        # Log entries are inserted as complete lines.  Older builds kept a
        # typewriter QTimer here, but the animation was intentionally removed
        # to reduce CPU/GIL pressure.  Keeping the old timer callback caused a
        # startup crash because _step no longer exists.
        self._ai_name_lc = "safu"   # updated when assistant name changes
    ...
    def _enqueue(self, text: str):
        """Append one complete coloured log line.

        The old character-by-character typewriter animation created hundreds of
        QTextDocument edits per response.  That looked flashy but competed with
        audio and heavy file/research work for the Python GIL.  Safu now logs in
        one insert: cleaner, faster and essentially free while idle.
        """
        tl = text.lower()
        _ai_pfx = f"{self._ai_name_lc}:"
        if tl.startswith("you:"):
            tag = "you"
        elif tl.startswith(_ai_pfx) or tl.startswith("safu:"):
            tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for SAFU", str(Path.home()),
            "All Files (*.*);;"
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure SAFU before first boot.", 9, color=C.PRI_DIM))
        layout.addSpacing(6)
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="SAFU", user_name="",
        ...
        lay.addSpacing(4)
        lay.addWidget(_lbl("ASSISTANT VOICE  •  Safu default: Leda (youthful)", 8,
                            color=C.TEXT_DIM, align=Qt.AlignmentFlag.AlignLeft))
        self._sel_voice = (voice or DEFAULT_VOICE)
        if self._sel_voice not in AVAILABLE_VOICES:
            self._sel_voice = DEFAULT_VOICE
        self._voice_combo = QComboBox()
        self._voice_combo.addItems(AVAILABLE_VOICES)
        self._voice_combo.setCurrentText(self._sel_voice)
        self._voice_combo.setFixedHeight(32)
        self._voice_combo.setFont(QFont("Segoe UI", 9))
        self._voice_combo.setStyleSheet(f"""
            QComboBox {{ background:{C.PANEL2}; color:{C.TEXT}; border:1px solid {C.BORDER};
        ...
        voice_note = _lbl("Original expressive AI voice. No copyrighted anime voice clips are used.", 7,
                          color=C.TEXT_DIM, align=Qt.AlignmentFlag.AlignLeft)
        voice_note.setWordWrap(True)
        lay.addWidget(voice_note)
        language_note = _lbl("INPUT  •  Any language (auto-detect)   |   SAFU REPLIES  •  English only", 8, color=C.PRI, align=Qt.AlignmentFlag.AlignLeft)
        language_note.setWordWrap(True)
        lay.addWidget(language_note)
    ...
    def _on_voice_pick(self, name: str):
        self._sel_voice = (name or "Leda").strip()
    ...
    def _save(self):
        name = self._name_input.text().strip() or "SAFU"
        user = self._user_input.text().strip()
        self.saved.emit(name, user, self._sel_color or DEFAULT_UI_COLOR, self._sel_voice)
        self.hide()


class ModelBrainOverlay(QWidget):
    """Configure SAFU's secondary reasoning models without editing JSON by hand."""
    ...
    def __init__(self, parent=None):
        ...
        self.setAttribute(Qt.WidgetAttribute.WA_StyledBackground, True)
        self.setStyleSheet(f"""
            ModelBrainOverlay {{ background: rgba(0, 6, 10, 248); border: 1px solid {C.BORDER_B}; border-radius: 6px; }}
        """)
        lay = QVBoxLayout(self); lay.setContentsMargins(24, 18, 24, 18); lay.setSpacing(7)

        def lbl(text, fs=8, color=C.TEXT_DIM):
            w=QLabel(text); w.setFont(QFont("Segoe UI", fs)); w.setStyleSheet(f"color:{color}; background:transparent;"); return w
        def edit(text="", secret=False, placeholder=""):
            e=QLineEdit(text); e.setPlaceholderText(placeholder); e.setFixedHeight(30)
            if secret: e.setEchoMode(QLineEdit.EchoMode.Password)
            e.setStyleSheet(f"QLineEdit {{background:#000d12;color:{C.TEXT};border:1px solid {C.BORDER};border-radius:4px;padding:4px 8px;}} QLineEdit:focus{{border-color:{C.PRI};}}")
            return e

        title=QLabel("🧠  MODEL BRAIN"); title.setFont(QFont("Segoe UI", 12, QFont.Weight.Bold)); title.setStyleSheet(f"color:{C.PRI}; background:transparent;"); lay.addWidget(title)
        note=lbl("Gemini Live remains the realtime voice/orchestrator. Heavy tasks can be delegated to the provider below.", 8, C.TEXT_MED); note.setWordWrap(True); lay.addWidget(note)

        lay.addWidget(lbl("PREFERRED SECONDARY BRAIN"))
        self.provider=QComboBox(); self.provider.addItems(["auto","gemini","openai","anthropic","ollama","openai_compatible"]); self.provider.setCurrentText(cfg["provider"]); self.provider.setFixedHeight(30); lay.addWidget(self.provider)

        lay.addWidget(lbl("OPENAI MODEL")); self.openai_model=edit(cfg["openai_model"]); lay.addWidget(self.openai_model)
        lay.addWidget(lbl("OPENAI API KEY (leave blank to keep current)")); self.openai_key=edit("", True, "sk-…"); lay.addWidget(self.openai_key)

        lay.addWidget(lbl("ANTHROPIC MODEL")); self.anthropic_model=edit(cfg["anthropic_model"]); lay.addWidget(self.anthropic_model)
        lay.addWidget(lbl("ANTHROPIC API KEY (leave blank to keep current)")); self.anthropic_key=edit("", True, "sk-ant-…"); lay.addWidget(self.anthropic_key)

        lay.addWidget(lbl("LOCAL SERVER TYPE")); self.local_provider=QComboBox(); self.local_provider.addItems(["ollama","openai_compatible"]); self.local_provider.setCurrentText("openai_compatible" if cfg["local_provider"] == "openai" else "ollama"); self.local_provider.setFixedHeight(30); lay.addWidget(self.local_provider)
        lay.addWidget(lbl("LOCAL SERVER URL")); self.local_url=edit(cfg["local_url"]); lay.addWidget(self.local_url)
        lay.addWidget(lbl("LOCAL MODEL")); self.local_model=edit(cfg["local_model"]); lay.addWidget(self.local_model)

        row=QHBoxLayout(); save=QPushButton("APPLY MODEL SETTINGS"); cancel=QPushButton("CANCEL")
        for b in (save,cancel): b.setFixedHeight(32); b.setCursor(Qt.CursorShape.PointingHandCursor)
        save.setStyleSheet(f"QPushButton{{color:{C.PRI};border:1px solid {C.PRI_DIM};background:transparent;border-radius:4px;}} QPushButton:hover{{background:{C.PRI_GHO};}}")
        cancel.setStyleSheet(f"QPushButton{{color:{C.TEXT_MED};border:1px solid {C.BORDER};background:transparent;border-radius:4px;}}")
        save.clicked.connect(self._save); cancel.clicked.connect(self.hide); row.addWidget(save); row.addWidget(cancel); lay.addLayout(row)

    def _save(self):
        from memory.config_manager import save_brain_settings, save_openai_key, save_anthropic_key
        save_brain_settings(provider=self.provider.currentText(), openai_model=self.openai_model.text(),
                            anthropic_model=self.anthropic_model.text(), local_provider=self.local_provider.currentText(),
                            local_url=self.local_url.text(), local_model=self.local_model.text())
        if self.openai_key.text().strip(): save_openai_key(self.openai_key.text().strip())
        if self.anthropic_key.text().strip(): save_anthropic_key(self.anthropic_key.text().strip())
        self.saved.emit(self.provider.currentText()); self.hide()
...
class AudioDeviceOverlay(_HudOverlay):
    """Choose which microphone SAFU listens to and which speakers it uses.
    ...
    is plugged in. 'SAFU can't hear me' is usually 'SAFU is listening to the
    webcam'."""
    ...
    def __init__(self, parent=None):
        ...
        self._in_box  = _row("MICROPHONE — what SAFU hears you with",
                             "input", get_input_device())
        lay.addSpacing(4)
        self._out_box = _row("SPEAKERS — what SAFU talks through",
...
class MemoryOverlay(_HudOverlay):
    """Everything SAFU has stored about you, and when it learned it.
    ...
    def _rebuild(self):
        ...
        hdr = QLabel("🧠  WHAT SAFU REMEMBERS")
        hdr.setFont(QFont("Courier New", 12, QFont.Weight.Bold))
        hdr.setStyleSheet(f"color: {C.PRI}; background: transparent;")
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick Safu actions."""
...
class PluginSettingsOverlay(QWidget):
    ...
    def _build_section(self, form: QVBoxLayout, sec: dict):
        ...
        if isinstance(action, dict) and callable(action.get("run")):
            ...
            ab.setFont(QFont("Segoe UI", 8, QFont.Weight.DemiBold))
...
class RemoteKeyOverlay(QWidget):
    ...
    def __init__(self, url: str, key: str, auto_login_url: str = "",
        ...
        new_btn.setFont(QFont("Segoe UI", 8, QFont.Weight.DemiBold))
        ...
        close_btn.setFont(QFont("Segoe UI", 8, QFont.Weight.DemiBold))
        close_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        close_btn.setStyleSheet(f"""
            QPushButton {{
                background: transparent; color: {C.TEXT_MED};
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected — SAFU ready")
...
class MainWindow(QMainWindow):
    ...
    _phone_connected_sig = pyqtSignal()       # dashboard callback -> Qt thread
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "SAFU").strip()
        ...
        self.on_interrupt      = None   # callable: () -> None — stop SAFU mid-speech
        self.on_voice_change   = None   # callable: () -> None — rebuild session with new voice
        self.on_audio_device_change = None  # callable: () -> None — reopen audio streams
        self._confirm_overlay  = None   # live ConfirmBanner, if one is on screen
        self.get_plugins       = None   # callable: () -> list[dict], set by SafuLive
        self.get_plugin_settings = None # callable: () -> list[dict] settings schemas, set by SafuLive
        self.on_wake_toggle    = None   # callable: (enable: bool) -> str, set by SafuLive
        ...
        _cam_title.setFont(QFont("Segoe UI", 8, QFont.Weight.DemiBold))
        _cam_title.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        _cam_hdr.addWidget(_cam_title)
        _cam_hdr.addStretch()
        _cam_x = QPushButton("✕  CLOSE")
        _cam_x.setFont(QFont("Segoe UI", 8, QFont.Weight.DemiBold))
    ...
    # ------------------------------------------------------------------
    # Icon generation — character portrait, matching the Safu interface
    # ------------------------------------------------------------------
    @staticmethod
    def _build_safu_icon(out_path: Path) -> bool:
        """Build a multi-resolution icon from the bundled original Safu avatar."""
        try:
            from PIL import Image, ImageDraw, ImageOps
        except ImportError:
            return False
        try:
            src = Path(__file__).resolve().parent / "config" / "safu_avatar.png"
            if not src.exists():
                return False
            im = Image.open(src).convert("RGBA")
            # Portrait crop: keep the face/head area rather than shrinking the
            # whole body into an unreadable desktop icon.
            w, h = im.size
            crop = im.crop((int(w * 0.17), int(h * 0.01), int(w * 0.86), int(h * 0.70)))
            crop = ImageOps.contain(crop, (900, 900), Image.Resampling.LANCZOS)

            canvas = Image.new("RGBA", (1024, 1024), (8, 7, 10, 255))
            d = ImageDraw.Draw(canvas)
            d.rounded_rectangle((24, 24, 1000, 1000), radius=190,
                                fill=(17, 14, 18, 255), outline=(217, 182, 93, 255), width=18)
            # Soft sakura-pink inset.
            d.rounded_rectangle((55, 55, 969, 969), radius=165,
                                outline=(239, 166, 188, 115), width=8)
            x = (1024 - crop.width) // 2
            y = 1024 - crop.height + 18
            canvas.alpha_composite(crop, (x, y))

            sizes = [256, 128, 64, 48, 32, 16]
            frames = [canvas.resize((n, n), Image.Resampling.LANCZOS) for n in sizes]
    ...
    def _create_lnk_windows(lnk: str, target: str, args: str,
        ...
        try:
            ...
            sc.Description      = "SAFU AI Assistant"
    ...
    def _create_desktop_shortcut(self):
        ...
        # Safu portrait icon (.ico — also exported as .png for Linux/macOS)
        ico_path = Path(__file__).resolve().parent / "config" / "safu.ico"
        if not ico_path.exists():
            self._build_safu_icon(ico_path)
        ...
        try:
            ...
            # ── macOS — proper .app bundle (no Terminal window) ───────────────
                ...
                (app / "Contents" / "Info.plist").write_text(
                    ...
                    '  <key>CFBundleExecutable</key><string>SAFU</string>\n'
                    '  <key>CFBundleIdentifier</key>'
                    '<string>com.safu.assistant</string>\n'
                    '  <key>CFBundleName</key><string>SAFU</string>\n'
    ...
    def _build_header(self) -> QWidget:
        ...
        self._title_lbl.setFont(QFont("Segoe UI", 22, QFont.Weight.Bold))
        self._title_lbl.setStyleSheet(f"color: {C.WHITE}; background: transparent;")
        brand.addWidget(self._title_lbl)
        self._sub_lbl = QLabel("PERSONAL AI  •  BUILT BY BASIT")
        self._sub_lbl.setFont(QFont("Segoe UI", 8, QFont.Weight.DemiBold))
        self._sub_lbl.setStyleSheet(f"color: {C.PRI}; background: transparent; letter-spacing: 1px;")
        brand.addWidget(self._sub_lbl)
        lay.addLayout(brand)

        wake_chip = QLabel("  ◉  HEY SAFU  ")
        wake_chip.setFont(QFont("Segoe UI", 8, QFont.Weight.DemiBold))
        wake_chip.setStyleSheet(
            f"color:{C.GREEN}; background:#0d1712; border:1px solid {C.GREEN_D}; "
            "border-radius:10px; padding:4px 7px;"
        )
        lay.addWidget(wake_chip)
        lay.addStretch()

        self._drawer_btn = QPushButton("☰  Control Center")
        self._drawer_btn.setCheckable(True)
        self._drawer_btn.setFixedHeight(34)
        self._drawer_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self._drawer_btn.setFont(QFont("Segoe UI", 9, QFont.Weight.DemiBold))
        self._drawer_btn.setStyleSheet(f"""
            QPushButton {{ color:{C.TEXT}; background:{C.PANEL2}; border:1px solid {C.BORDER};
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        hdr.setStyleSheet(f"color:{C.PRI}; background:transparent; padding:0 0 5px 2px;")
        lay.addWidget(hdr)
        sub = QLabel("Quiet monitoring • 5s refresh")
        ...
        self._bar_tmp = MetricBar("TEMP", C.MUTED_C)
        for bar in (self._bar_cpu, self._bar_mem, self._bar_net, self._bar_gpu, self._bar_tmp):
            lay.addWidget(bar)

        info_panel = QWidget()
        info_panel.setStyleSheet(f"background:{C.PANEL2}; border:1px solid {C.BORDER}; border-radius:12px;")
        ip = QVBoxLayout(info_panel); ip.setContentsMargins(10, 9, 10, 9); ip.setSpacing(5)
        ...
        osl = QLabel(f"OS  {os_name}")
        osl.setFont(QFont("Segoe UI", 8)); osl.setStyleSheet(f"color:{C.ACC2}; background:transparent; border:none;")
        ip.addWidget(osl)
        lay.addWidget(info_panel)

        lay.addStretch()
        tip = QLabel("SAFU works best when\nheavy features stay off\nuntil you need them.")
        tip.setWordWrap(True)
        tip.setFont(QFont("Segoe UI", 8))
        tip.setStyleSheet(f"color:{C.TEXT_DIM}; background:{C.PANEL2}; border:1px solid {C.BORDER}; border-radius:12px; padding:10px;")
        lay.addWidget(tip)
        return w
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self._interrupt_btn = QPushButton("■  Stop current response   [Esc]")
        self._interrupt_btn.setFixedHeight(34)
        self._interrupt_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self._interrupt_btn.setFont(QFont("Segoe UI", 9, QFont.Weight.DemiBold))
        self._interrupt_btn.setStyleSheet(f"""
            QPushButton {{ background:#1a0d12; color:{C.MUTED_C}; border:1px solid #5a2938;
                border-radius:10px; text-align:left; padding:0 11px; }}
            QPushButton:hover {{ border-color:{C.MUTED_C}; background:#241018; }}
        """)
        ...
        fh = QLabel("FILES")
        fh.setFont(QFont("Segoe UI", 8, QFont.Weight.Bold)); fh.setStyleSheet(f"color:{C.TEXT_MED}; background:transparent;")
        lay.addWidget(fh)
        self._drop_zone = FileDropZone()
        self._drop_zone.file_selected.connect(self._on_file_selected)
        lay.addWidget(self._drop_zone)
        self._file_hint = QLabel("Drop a file here, or ask Safu to open one from C:, D:, E: or USB drives.")
        ...
        lh = QLabel("ACTIVITY")
        lh.setFont(QFont("Segoe UI", 8, QFont.Weight.Bold)); lh.setStyleSheet(f"color:{C.TEXT_MED}; background:transparent;")
        lay.addWidget(lh)
        self._log = LogWidget()
    ...
    def _build_quick_drawer(self) -> QWidget:
        ...
        model_btn = QPushButton("🧠  MODEL BRAIN")
        model_btn.setFixedHeight(26)
        model_btn.setFont(QFont("Segoe UI", 7))
        model_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        model_btn.setStyleSheet(_BTN_STYLE_DIM)
        model_btn.clicked.connect(self._open_model_brain)
    ...
    def _build_input_row(self) -> QHBoxLayout:
        ...
        self._input.setPlaceholderText("Ask Safu anything…")
        self._input.setFont(QFont("Segoe UI", 10))
        self._input.setFixedHeight(38)
        self._input.setStyleSheet(f"""
            QLineEdit {{ background:{C.PANEL2}; color:{C.WHITE}; border:1px solid {C.BORDER};
        ...
        send.setStyleSheet(f"""
            QPushButton {{ background:{C.PRI}; color:#17120b; border:none; border-radius:11px; }}
    ...
    def _build_content_panel(self) -> QWidget:
        ...
        dot = QLabel("◈")
        dot.setFont(QFont("Segoe UI", 9, QFont.Weight.DemiBold))
    ...
    def _build_footer(self) -> QWidget:
        ...
        left = QLabel("F4  Mute   •   F11  Fullscreen   •   Esc  Interrupt")
        left.setFont(QFont("Segoe UI", 7)); left.setStyleSheet(f"color:{C.TEXT_DIM}; background:transparent;")
        lay.addWidget(left); lay.addStretch()
        right = QLabel("SAFU  •  PERSONAL AI  •  BUILT BY BASIT")
        right.setFont(QFont("Segoe UI", 7, QFont.Weight.DemiBold)); right.setStyleSheet(f"color:{C.PRI_DIM}; background:transparent;")
        lay.addWidget(right)
        return w
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                try:
                    winreg.QueryValueEx(key, "SAFU_AI")
                    return True
                ...
                return (Path.home() / "Library" / "LaunchAgents"
                        / "com.safu.assistant.plist").exists()
            else:
                return (Path.home() / ".config" / "autostart" / "safu.desktop").exists()
        except Exception:
            return False
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    winreg.DeleteValue(reg, "SAFU_AI")
                else:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "SAFU_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
                plist_dir = Path.home() / "Library" / "LaunchAgents"
                plist_dir.mkdir(parents=True, exist_ok=True)
                plist = plist_dir / "com.safu.assistant.plist"
                if currently_on:
                    ...
                    plist.write_text(
                        ...
                        '"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
                        '<plist version="1.0"><dict>\n'
                        '  <key>Label</key><string>com.safu.assistant</string>\n'
    ...
    def _wake_state(self) -> dict:
        """Return wake state. Hey Safu always has a no-download cloud fallback."""
        if self.wake_get_state:
            try:
                st = self.wake_get_state()
                return {"ready": True,
                        "enabled": bool(st.get("enabled")),
                        "awake": bool(st.get("awake")),
                        "mode": st.get("mode", "cloud")}
            except Exception:
                pass
        enabled = False
        mode = "cloud"
        try:
            ...
            mode = "local" if is_ready() else "cloud"
        except Exception:
            pass
        return {"ready": True, "enabled": enabled, "awake": True, "mode": mode}
    ...
    def _refresh_wake_btns(self):
        ...
        st = self._wake_state()
        _on = f"""
            QPushButton {{ background: #0d1712; color: {C.GREEN};
                border: 1px solid {C.GREEN_D}; border-radius: 8px;
                text-align: left; padding: 0 9px; }}
            QPushButton:hover {{ background: #122019; }}"""
        _off = f"""
            QPushButton {{ background: transparent; color: {C.TEXT_DIM};
                border: 1px solid {C.BORDER}; border-radius: 8px;
                text-align: left; padding: 0 9px; }}
            QPushButton:hover {{ color: {C.TEXT}; border-color: {C.BORDER_B}; }}"""
        mode = "LOCAL" if st.get("mode") == "local" else "SMART"
        self._wake_btn.setEnabled(True)
        if st["enabled"]:
            self._wake_btn.setText(f"🎙  HEY SAFU: ON  •  {mode}")
            ...
            self._wake_btn.setText("🎙  HEY SAFU: OFF")
    ...
    def _toggle_wake_word(self):
        ...
        if self.on_wake_toggle:
            try:
                ...
                self._log_sig.emit(f"ERR: Wake-word toggle failed — {e}")
        self._refresh_wake_btns()

    def _on_wake_install_done(self, ok: bool, msg: str):
        self._log_sig.emit(f"SYS: {'Local wake engine ready.' if ok else 'Local wake repair failed: ' + msg}")
        self._refresh_wake_btns()
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "SAFU") or "SAFU",
            cfg.get("user_name", ""),
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = "",
        ...
        self._assistant_name = name.strip() or "SAFU"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"{display} — {APP_VERSION}")
        self._title_lbl.setText(display)
        if display == "SAFU":
            self._sub_lbl.setText("Personal AI • Built by Basit")
        else:
    ...
    def _open_model_brain(self):
        ov = ModelBrainOverlay(parent=self.centralWidget())
        ov.saved.connect(lambda p: self._log.append_log(f"SYS: Secondary model brain set — {p}"))
        self._centre_overlay(ov)
        self._model_brain_overlay = ov
    ...
    def _check_config(self) -> bool:
        ...
        try:
            ...
            return bool(get_gemini_key()) and bool(d.get("os_system"))
        except Exception:
            return False
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        save_api_keys(key)
        # Merge the OS into the existing config instead of wiping Safu's name,
        # voice, theme, wake setting and other preferences on first setup.
        data = _read_full_config()
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "SAFU") or "SAFU"
